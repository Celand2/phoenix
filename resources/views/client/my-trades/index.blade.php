...
<article class="rounded-lg border border-ash-600 bg-ash-800 p-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-lg font-bold">{{ $userTrade->trade?->name }}</h2>
            <p class="text-sm text-ash-200">{{ $userTrade->category?->name }} — {{ ucfirst($userTrade->status) }}</p>
        </div>
        <p class="text-gold-400 font-bold">{{ \App\Support\Money::formatForUser($userTrade->daily_gain, auth()->user()) }}/jour</p>
    </div>
...
@empty
<p class="rounded-lg border border-ash-600 bg-ash-800 p-5 text-center text-ash-200">Vous n'avez aucun trade pour le moment.</p>
@endforelse
