...
<article class="rounded-lg border border-ash-600 bg-ash-800 p-5">
    ...
    <p class="mt-3 text-2xl font-bold text-crimson-400">{{ \App\Support\Money::formatForUser($trade->amount, auth()->user()) }}</p>
    <p class="text-sm text-gold-400">Gain journalier: {{ \App\Support\Money::formatForUser($trade->dailyGain(), auth()->user()) }}</p>
