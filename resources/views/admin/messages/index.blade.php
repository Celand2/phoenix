...
<div class="mt-8">
    <div class="overflow-x-auto rounded-lg border border-ash-200 bg-white shadow-sm"><table class="w-full text-left text-sm"><thead class="bg-ash-50 text-ash-600 border-b border-ash-200"><tr><th class="p-3">Client</th><th>Sujet</th><th>From</th><th></th></tr></thead><tbody>@foreach($messages as $message)<tr class="border-t border-ash-100 hover:bg-ash-50 transition-colors"><td class="p-3">{{ $message->user?->name }}</td><td>{{ $message->subject }}</td><td>{{ $message->from }}</td><td><div class="flex gap-2"><a class="text-crimson-600 font-medium" href="{{ route('admin.messages.show', $message) }}">Voir</a><form action="{{ route('admin.messages.destroy', $message) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer ce message ?')">@csrf @method('DELETE')<button type="submit" class="text-ash-400 font-medium hover:underline">Supprimer</button></form></div></td></tr>@endforeach</tbody></table></div>{{ $messages->links() }}
</div>
